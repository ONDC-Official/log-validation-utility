...
const checksReportData = (data: any) => {
    const rsfObj: any = {};

    // Check if the data object is empty or undefined
    if (!data || isObjectEmpty(data)) {
        return { [RSFapiSequence.REPORT]: 'JSON cannot be empty' };
    }

    try {
        logger.info(`Validating Schema for ${constants.REPORT} API`);
        const vs = validateSchema('rsf', constants.REPORT, data);

        if (vs !== 'error') {
            Object.assign(rsfObj, vs);
        } else {
            return { schemaError: 'Schema validation failed for /report' };
        }
        ...
        logger.info('REPORT data passed all checks');
        return rsfObj;
    } catch (err: any) {
        if (err.code === 'ENOENT') {
            logger.info(`!!File not found for /${constants.REPORT} API!`);
        } else {
            logger.error(`!!Some error occurred while checking /${constants.REPORT} API`, err);
        }
    }
};

export default checksReportData;
