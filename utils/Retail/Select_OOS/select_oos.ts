...
export const checkSelect_OOS = (data: any, msgIdSet: any) => {
  ...
  try {
    ...
    select_oos.items.forEach((item: { id: string | number; quantity: { count: number } }) => {
  ...
  const checksOnValidProvider = (provider: any) => {
    ...
    try {
      ...
      const providerID = select_oos.provider.id
      const items = select_oos.items
  ...
  if (providerOnSelect) {
    ...
    errorObj.providerChecks = `Warning: Missed checks for provider as provider with  ID: ${select_oos.provider.id} does not exist on ${constants.ON_SEARCH} API`
  }
