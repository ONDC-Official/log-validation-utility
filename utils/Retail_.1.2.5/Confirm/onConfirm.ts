...
export const checkOnConfirm = (data: any, flow: string, schemaValidation?: boolean, stateless?: boolean) => {
  ...
  if (!data || isObjectEmpty(data)) {
    businessErrors[ApiSequence.ON_CONFIRM] = 'JSON cannot be empty'
    return { businessErrors }
  }

  const { message, context }: any = data
  if (!message || !context || !message.order || isObjectEmpty(message) || isObjectEmpty(message.order)) {
    businessErrors.missingFields = '/context, /message, /order or /message/order is missing or empty'
    return { businessErrors }
  }

  // Schema validation
  const schemaValidationResult =
    schemaValidation !== false ? validateSchemaRetailV2(context.domain.split(':')[1], constants.ON_CONFIRM, data) : 'skip'

  if (schemaValidationResult !== 'error' && schemaValidationResult !== 'skip') {
    Object.assign(schemaErrors, schemaValidationResult)
  }

  // Basic context and ID validations
  const contextRes: any = checkContext(context, constants.ON_CONFIRM)
  const checkBap = checkBppIdOrBapId(context.bap_id)
  const checkBpp = checkBppIdOrBapId(context.bpp_id)

  if (checkBap) businessErrors.bap_id = 'context/bap_id should not be a url'
  if (checkBpp) businessErrors.bpp_id = 'context/bpp_id should not be a url'

  if (!contextRes?.valid) {
    Object.assign(businessErrors, contextRes.ERRORS)
  }

  // Message ID and domain comparison (only if not stateless)
  if (!stateless) {
    try {
      ...
      if (!_.isEqual(getValue(`${ApiSequence.CONFIRM}_msgId`), context.message_id)) {
        businessErrors[`${ApiSequence.ON_CONFIRM}_msgId`] =
          `Message Ids for /${constants.CONFIRM} and /${constants.ON_CONFIRM} api should be same`
      }
    } catch (error: any) {
      logger.error(`!!Error while checking message id for /${constants.ON_CONFIRM}, ${error.stack}`)
      businessErrors.message_id = `Error while checking message id for /${constants.ON_CONFIRM}: ${error.message}`
    }
  ...
  try {
    ...
    if (!_.isEqual(searchContext.city, context.city)) {
      businessErrors.city = `City code mismatch in /${constants.SEARCH} and /${constants.ON_CONFIRM}`
    }
  } catch (error: any) {
    logger.info(`Error while comparing city in /${constants.SEARCH} and /${constants.ON_CONFIRM}, ${error.stack}`)
    businessErrors.city = `Error while comparing city in /${constants.SEARCH} and /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (_.gte(tmpstmp, context.timestamp)) {
      businessErrors.tmpstmp = `Timestamp for /${constants.CONFIRM} api cannot be greater than or equal to /${constants.ON_CONFIRM} api`
    } else {
      const timeDiff = timeDifference(context.timestamp, tmpstmp)
      logger.info(timeDiff)
      if (timeDiff > 5000) {
        businessErrors.tmpstmp = `context/timestamp difference between /${constants.ON_CONFIRM} and /${constants.CONFIRM} should be less than 5 sec`
      }
    }
    ...
    businessErrors.timestamp = `Error while comparing timestamp for /${constants.CONFIRM} and /${constants.ON_CONFIRM}: ${error.message}`
  }

  try {
    logger.info(`Comparing transaction Ids of /${constants.SELECT} and /${constants.ON_CONFIRM}`)
    if (!_.isEqual(getValue('txnId'), context.transaction_id)) {
      businessErrors.txnId = `Transaction Id should be same from /${constants.SELECT} onwards`
    }
  } catch (error: any) {
    logger.error(
      `!!Error while comparing transaction ids for /${constants.SELECT} and /${constants.ON_CONFIRM} api, ${error.stack}`,
    )
    businessErrors.transaction_id = `Error while comparing transaction ids for /${constants.SELECT} and /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (OnInitCancellationTerms !== undefined) {
      ...
      if (Errors) {
        ...
        while (i < len) {
          ...
          businessErrors[key] = `${Errors[i]} when compared with on_init Cancellation terms`
          i++
        }
      }
    }
    ...
    businessErrors.cancellation_terms = `Error while checking Cancellation terms for /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    message.order.fulfillments.forEach((fulfillment: any) => {
      if (!fulfillment['@ondc/org/TAT']) {
        businessErrors[`message.order.fulfillments[${fulfillment.id}]`] =
      ...
      if (
        ...
        businessErrors[`TAT_Mismatch`] =
          `TAT Mistatch between  /${constants.ON_CONFIRM} i.e ${isoDurToSec(
            fulfillment['@ondc/org/TAT'],
          )} seconds & /${constants.ON_SELECT} i.e ${on_select_fulfillment_tat_obj[fulfillment_id]} seconds`
      }
    })
  } catch (error: any) {
    logger.error(`!!Error while Checking fulfillment ids for  /${constants.ON_CONFIRM}, ${error.stack}`)
    businessErrors.fulfillments = `Error while Checking fulfillment ids for /${constants.ON_CONFIRM}: ${error.message}`
  }

  try {
    logger.info(`Comparing order ids in /${constants.CONFIRM} and /${constants.ON_CONFIRM}`)
    if (getValue('cnfrmOrdrId') != on_confirm.id) {
      businessErrors.orderID = `Order Id mismatches in /${constants.CONFIRM} and /${constants.ON_CONFIRM}`
    }
  } catch (error: any) {
    logger.error(`!!Error while trying to fetch order ids in /${constants.ON_CONFIRM}, ${error.stack}`)
    businessErrors.order_id = `Error while trying to fetch order ids in /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (on_confirm.state === 'Created' || on_confirm.state === 'Accepted') {
      if (cnfrmOrdrCrtd && (!on_confirm.created_at || on_confirm.created_at != cnfrmOrdrCrtd)) {
        businessErrors.crtdtmstmp = `order.created_at timestamp mismatches in /${constants.CONFIRM} and /${constants.ON_CONFIRM}`
      }
      ...
      if (
        ...
        businessErrors.updtdtmstmp = `order.updated_at timestamp should be updated as per the context.timestamp (since default fulfillment state is added)`
      }
    }
  } catch (error: any) {
    logger.error(`!!Error while checking order timestamps in /${constants.ON_CONFIRM}, ${error.stack}`)
    businessErrors.order_timestamps = `Error while checking order timestamps in /${constants.ON_CONFIRM}: ${error.message}`
  }

  try {
    logger.info(`Checking provider id and location in /${constants.ON_CONFIRM}`)
    if (on_confirm.provider.id != getValue('providerId')) {
      businessErrors.prvdrId = `Provider Id mismatches in /${constants.ON_SEARCH} and /${constants.ON_CONFIRM}`
    }

    if (on_confirm.provider.locations[0].id != getValue('providerLoc')) {
      businessErrors.prvdrLoc = `provider.locations[0].id mismatches in /${constants.ON_SEARCH} and /${constants.ON_CONFIRM}`
    }
  } catch (error: any) {
    logger.error(`!!Error while checking provider id and location in /${constants.ON_CONFIRM}, ${error.stack}`)
    businessErrors.provider = `Error while checking provider id and location in /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    while (i < len) {
      ...
      if (checkItemTag(item, select_customIdArray)) {
        ...
        businessErrors[itemkey] = `items[${i}].tags.parent_id mismatches for Item ${itemId} in /${constants.SELECT} and /${constants.INIT}`
      }

      if (parentItemIdSet && item.parent_item_id && !parentItemIdSet.includes(item.parent_item_id)) {
        const itemkey = `item_PrntItmId${i}`
        businessErrors[itemkey] = `items[${i}].parent_item_id mismatches for Item ${itemId} in /${constants.ON_SEARCH} and /${constants.ON_INIT}`
      }

      if (itemId in itemFlfllmnts) {
        const validFfIds = Array.isArray(itemFlfllmnts[itemId]) ? itemFlfllmnts[itemId] : [itemFlfllmnts[itemId]]

        if (!validFfIds.includes(on_confirm.items[i].fulfillment_id)) {
          const itemkey = `item_FFErr${i}`
          businessErrors[itemkey] = `items[${i}].fulfillment_id (${on_confirm.items[i].fulfillment_id}) does not match any valid fulfillment_id for Item ${itemId} in /${constants.ON_SELECT}`
        }
      } else {
        const itemkey = `item_FFErr${i}`
        businessErrors[itemkey] = `Item Id ${itemId} does not exist in /on_select`
      }

      if (itemId in itemsIdList) {
        if (on_confirm.items[i].quantity.count != itemsIdList[itemId]) {
          businessErrors.countErr = `Warning: items[${i}].quantity.count for item ${itemId} mismatches with the items quantity selected in /${constants.SELECT}`
        }
      }
    ...
    businessErrors.items = `Error while comparing Item and Fulfillment Id in /${constants.ON_SELECT} and /${constants.CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    businessErrors.fulfillment_storage = `Error while storing fulfillment: ${error.message}`
  }
  ...
  if (Array.isArray(fulfillments)) {
    fulfillments.forEach((fulfillment, index) => {
      ...
      if (type === SELF_PICKUP && category === KERBSIDE) {
        if (!vehicle) {
          businessErrors[`fulfillment${index}_vehicle`] =
          ...
          businessErrors[`fulfillment${index}_vehicle_registration`] =
  ...
  try {
    ...
    if (np_type_arr.length > 0) {
      ...
      businessErrors[key] = `np_type not found in on_confirm`
    }

    if (accept_bap_terms.length > 0) {
      const key = 'message.order.tags[0].list'
      businessErrors[key] = `accept_bap_terms is not required for now!`
    }

    if (np_type && np_type != np_type_on_search) {
      const key = 'message.order.tags[0].list'
      businessErrors[key] = `np_type of on_search is not same to np_type of on_confirm`
    }
    ...
    if (!_.isEmpty(bpp_terms_obj)) {
      ...
      list.map((item: any) => {
        if (item.code == 'tax_number') {
          if (item.value.length != 15) {
            const key = `message.order.tags[0].list`
            businessErrors[key] = `Number of digits in tax number in  message.order.tags[0].list should be 15`
          } else {
        ...
        if (item.code == 'provider_tax_number') {
          if (item.value.length != 10) {
            const key = `message.order.tags[0].list`
            businessErrors[key] = `Number of digits in provider tax number in  message.order.tags[0].list should be 10`
          } else {
      ...
      if (tax_number.length == 0) {
        ...
        businessErrors['tax_number'] = `tax_number must be present for ${constants.ON_CONFIRM}`
      } else {
        ...
        if (!taxNumberPattern.test(tax_number)) {
          ...
          businessErrors.tax_number = `Invalid format for tax_number in ${constants.ON_CONFIRM}`
        }
      }

      if (provider_tax_number.length == 0) {
        logger.error(`tax_number must present in ${constants.ON_CONFIRM}`)
        businessErrors['provider_tax_number'] = `provider_tax_number must be present for ${constants.ON_CONFIRM}`
      } else {
        ...
        if (!taxNumberPattern.test(provider_tax_number)) {
          ...
          businessErrors.provider_tax_number = `Invalid format for provider_tax_number in ${constants.ON_CONFIRM}`
        }
      }
      ...
      if (tax_number.length == 15 && provider_tax_number.length == 10) {
        ...
        if (pan_id != provider_tax_number && np_type_on_search == 'ISN') {
          businessErrors[`message.order.tags[0].list`] =
            `Pan_id is different in tax_number and provider_tax_number in message.order.tags[0].list`
          logger.error(
            'businessErrors[`message.order.tags[0].list`] = `Pan_id is different in tax_number and provider_tax_number in message.order.tags[0].list`',
          )
          ...
          logger.error(
            "businessErrors[`message.order.tags[0].list`] = `Pan_id shoudn't be same in tax_number and provider_tax_number in message.order.tags[0].list`",
          )
        }
      }
    }
    ...
    businessErrors.tax_info = `Error while comparing valid pan_id in tax_number and provider_tax_number: ${error.message}`
  }
  ...
  try {
    ...
    if (!_.isEqual(context.timestamp, on_confirm.updated_at)) {
      ...
      businessErrors[key] = `updated_at timestamp should be equal to  context timestamp for /${constants.ON_CONFIRM}`
    ...
    businessErrors.updated_at = `Error while compairing updated_at timestamp with context timestamp for ${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (billingErrors) {
      ...
      while (i < len) {
        ...
        businessErrors[key] = `${billingErrors[i]} when compared with init billing object`
        i++
      }
    }
  } catch (error: any) {
    logger.info(`!Error while comparing billing object in /${constants.CONFIRM} and /${constants.ON_CONFIRM}`)
    businessErrors.billing = `Error while comparing billing object in /${constants.CONFIRM} and /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    while (i < len) {
      ...
      if (on_confirm.fulfillments[i].id) {
        ...
        if (!Object.values(itemFlfllmnts).includes(id)) {
          const key = `ffID${id}`
          //MM->Mismatch
          businessErrors[key] = `fulfillment id ${id} does not exist in /${constants.ON_SELECT}`
        }
      } else {
        businessErrors.ffId = `fulfillments[${i}].id is missing in /${constants.ON_CONFIRM}`
      }

      if (!on_confirm.fulfillments[i].type) {
        const key = `ffID type`
        businessErrors[key] = `fulfillment type does not exist in /${constants.ON_SELECT}`
      }
      ...
      if (getValue(`${ffId}_tracking`)) {
        if (on_confirm.fulfillments[i].tracking === false || on_confirm.fulfillments[i].tracking === true) {
          if (getValue(`${ffId}_tracking`) != on_confirm.fulfillments[i].tracking) {
            ...
            businessErrors['ffTracking'] = `Fulfillment Tracking mismatch with the ${constants.ON_SELECT} call`
          }
        } else {
          logger.info(`Tracking must be present for fulfillment ID: ${ffId} in boolean form`)
          businessErrors['ffTracking'] = `Tracking must be present for fulfillment ID: ${ffId} in boolean form`
        }
      }
      ...
      if (!ffStateCheck) {
        const key = `ffState${i}`
        businessErrors[key] = `default fulfillments state is missing in /${constants.ON_CONFIRM}`
      }

      if (on_confirm.fulfillments[i].type === 'Self-Pickup') {
        if (!on_confirm.fulfillments[i].start) {
          businessErrors.ffstart = `fulfillments[${i}] start location is mandatory`
        }
      } else {
        if (!on_confirm.fulfillments[i].start || !on_confirm.fulfillments[i].end) {
          businessErrors.ffstartend = `fulfillments[${i}] start and end locations are mandatory`
        }
      }

      try {
        if (!compareCoordinates(on_confirm.fulfillments[i].start.location.gps, getValue('providerGps'))) {
          businessErrors.sellerGpsErr = `store gps location /fulfillments[${i}]/start/location/gps can't change`
        }
      } catch (error: any) {
        logger.error(`!!Error while checking store location in /${constants.ON_CONFIRM}, ${error.stack}`)
        businessErrors.sellerGpsErr = `Error while checking store location in /${constants.ON_CONFIRM}: ${error.message}`
      }

      try {
        if (!getValue('providerName')) {
          businessErrors.sellerNameErr = `Invalid store name inside fulfillments in /${constants.ON_CONFIRM}`
        } else if (
          !_.isEqual(on_confirm.fulfillments[i].start.location.descriptor.name, getValue('providerName'))
        ) {
          businessErrors.sellerNameErr = `store name  /fulfillments[${i}]/start/location/descriptor/name can't change`
        }
      } catch (error: any) {
        logger.error(`!!Error while checking store name in /${constants.ON_CONFIRM}`)
        businessErrors.sellerNameErr = `Error while checking store name in /${constants.ON_CONFIRM}: ${error.message}`
      }
      if (on_confirm.fulfillments[i].type !== 'Self-Pickup') {
        if (!_.isEqual(on_confirm.fulfillments[i].end.location.gps, getValue('buyerGps'))) {
          businessErrors.buyerGpsErr = `fulfillments[${i}].end.location gps is not matching with gps in /select`
        }

        if (!_.isEqual(on_confirm.fulfillments[i].end.location.address.area_code, getValue('buyerAddr'))) {
          businessErrors.gpsErr = `fulfillments[${i}].end.location.address.area_code is not matching with area_code in /select`
        }
      }
    ...
    businessErrors.fulfillments_object = `Error while checking fulfillments object in /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (parseFloat(on_confirm.payment.params.amount) != parseFloat(on_confirm.quote.price.value)) {
      businessErrors.onConfirmedAmount = `Quoted price (/${constants.ON_CONFIRM}) doesn't match with the amount in payment.params`
    }
  } catch (error: any) {
    logger.error(
      `!!Error while Comparing /${constants.ON_CONFIRM} quoted Price and Payment Params amount, ${error.stack}`,
    )
    businessErrors.onConfirmedAmount = `Error while Comparing /${constants.ON_CONFIRM} quoted Price and Payment Params amount: ${error.message}`
  }
  ...
  try {
    ...
    if (!sumQuoteBreakUp(on_confirm.quote)) {
      ...
      businessErrors[key] = `item quote breakup prices for ${constants.ON_CONFIRM} should be equal to the total price.`
    ...
    if (flow === FLOW.FLOW0099 || collect_payment === 'N' || flow === OFFERSFLOW.FLOW0098) {
      ...
      if (offers.length === 0) {
        businessErrors['offer-not-found'] = `Offer is required for the flow: ${flow}`
        ...
        offers.forEach((offer: any, index: number) => {
          ...
          if (!providerOffer) {
            businessErrors[`offer[${index}]`] = `Offer with id ${offer.id} is not available for the provider.`
            return
          }
          ...
          if (offerType === 'financing') {
            ...
            if (finance_txn) {
              ...
              if (financeTxnError) {
                ...
                while (i < len) {
                  const key = `financeTagsError${i}`
                  businessErrors[key] = `${financeTxnError[i]}`
                  i++
                }
              }
            }
    ...
    businessErrors.quote = `Error while Comparing Quote object for /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (flow !== FLOW.FLOW0099 && flow != OFFERSFLOW.FLOW0098) {
      ...
      if (hasItemWithQuantity) {
        ...
        while (i < len) {
          ...
          businessErrors[key] = `${quoteErrors[i]}`
          i++
        }
      }
    }
    ...
    businessErrors.quote_comparison = `Error while comparing quote in /${constants.ON_SELECT} and /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (oninitQuotePrice != onConfirmQuotePrice) {
      ...
      businessErrors.quoteErr = `Quoted Price in /${constants.CONFIRM} INR ${onConfirmQuotePrice} does not match with the quoted price in /${constants.ON_INIT} INR ${oninitQuotePrice}`
    }
    ...
    businessErrors.price_comparison = `Error while comparing order price value in /${constants.ON_INIT} and /${constants.CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (!_.isEqual(getValue('cnfrmpymnt'), on_confirm.payment)) {
      businessErrors.pymntObj = `payment object mismatches in /${constants.CONFIRM} & /${constants.ON_CONFIRM}`
    }
  } catch (error: any) {
    logger.error(
      `!!Error while comparing payment object in /${constants.CONFIRM} & /${constants.ON_CONFIRM}, ${error.stack}`,
    )
    businessErrors.payment_object = `Error while comparing payment object in /${constants.CONFIRM} & /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (
      ...
      businessErrors.buyerFF = `Buyer app finder fee can't change in /${constants.ON_CONFIRM}`
    ...
    businessErrors.buyer_app_finder_fee = `Error while comparing buyer app finder fee in /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (list_ON_CONFIRM && !list_ON_CONFIRM.some((data: any) => data.code == 'np_type')) {
      businessErrors['message/order/tags/bpp_terms/np_type'] =
    ...
    businessErrors.tax_number_match = `Error while matching the tax_number in ON_CONFIRM and ON_INIT: ${error.message}`
  }
  ...
  try {
    ...
    if (on_confirm.tags) {
      ...
      if (bap_terms === false) {
        businessErrors.tags_bap_terms = `Tags should have same and valid gst_number as passed in /${constants.CONFIRM}`
      }

      const bpp_terms = areGSTNumbersMatching(confirm_tags, on_confirm.tags, 'bpp_terms')
      if (bpp_terms === false) {
        businessErrors.tags_bpp_terms = `Tags should have same and valid gst_number as passed in /${constants.CONFIRM}`
      }
    }
  } catch (error: any) {
    logger.error(
    ...
    businessErrors.tags_comparison = `Error while Comparing tags in /${constants.CONFIRM} and /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  try {
    ...
    if (!status) {
      businessErrors['message/order/transaction_id'] = `Transaction_id missing in message/order/payment`
    }
  } catch (err: any) {
    logger.error(`Error while checking transaction is in message.order.payment`)
    businessErrors.transaction_id_payment = `Error while checking transaction_id in message.order.payment: ${err.message}`
  }
  ...
  try {
    if (flow === FLOW.FLOW012) {
      ...
      if (payment.status !== confirmPaymentStatus) {
        ...
        businessErrors[key] = `payment status in /${constants.ON_CONFIRM} must be same as in /${constants.CONFIRM} `
      }
      if (payment.status !== PAYMENT_STATUS.NOT_PAID) {
        ...
        businessErrors.pymntstatus = `Payment status should be ${PAYMENT_STATUS.NOT_PAID} for ${FLOW.FLOW012} flow (Cash on Delivery)`
      }
    }
  } catch (err: any) {
    logger.error('Error while checking payment in message/order/payment: ' + err.message)
    businessErrors.payment_status = `Error while checking payment in message/order/payment: ${err.message}`
  }
  ...
  try {
    ...
    businessErrors.bpp_terms_list = `Error while Checking if list provided in bpp_terms is same as provided in ${constants.ON_INIT}: ${err.message}`
  }
  ...
  try {
    ...
    for (const tag of tags) {
      if (tag.code === 'bap_terms') {
        ...
        if (hasStaticTerms) {
          businessErrors['message/order/tags/bap_terms/static_terms'] =
    ...
    businessErrors.bap_terms = `Error while Checking bap_terms in ${constants.ON_CONFIRM}: ${err.message}`
  }
  ...
  if (flow === FLOW.FLOW003) {
    ...
    if (!ele || !item) {
      ...
      businessErrors[key] = `fulfillments must be same as in /${constants.ON_INIT}`
    }
    if (item?.end?.time?.range && ele?.end?.time?.range) {
      ...
      if (itemRange.start !== eleRange.start && itemRange.end !== eleRange.end) {
        ...
        businessErrors[key] = `slots in fulfillments must be same as in /${constants.ON_INIT}`
      }
    }
  }
  ...
  try {
    ...
    if (flow === FLOW.FLOW017) {
      if (!expectedCreds || !credsWithProviderId || !confirmCreds) {
        businessErrors['MissingCreds'] = `creds must be present in /${constants.ON_SEARCH}`
      } else if (!deepCompare(expectedCreds, confirmCreds)) {
        businessErrors['MissingCreds'] = `creds must be present and same as in /${constants.ON_SEARCH}`
      }
    }
    if (credsWithProviderId && confirmCreds) {
      if (expectedCreds) {
        if (!deepCompare(expectedCreds, confirmCreds)) {
          businessErrors['MissingCreds'] = `Mismatch found in creds, it must be same as in /${constants.ON_SEARCH}`
        }
      }
    }
  } catch (err: any) {
    logger.error(`Error while Checking creds in ${constants.ON_CONFIRM}, ${err.stack} `)
    businessErrors.creds = `Error while Checking creds in ${constants.ON_CONFIRM}: ${err.message}`
  }
  ...
  if (flow === FLOW.FLOW01F) {
    ...
    if (!deepCompare(list, bppTerms)) {
      businessErrors['missingbppTerms'] = `bppTerms must be same as provided in /${constants.ON_SEARCH} call`
    }
  }
  ...
  try {
    ...
    if (orderState === 'Accepted') {
      ...
      if (!routingType) {
        businessErrors.routingType = `Routing type tag is mandatory in delivery fulfillment when order.state is 'Accepted' in /${constants.ON_CONFIRM}`
      } else if (!isValidRoutingType(routingType)) {
        businessErrors.routingType = `Invalid routing type '${routingType}'. Must be one of: P2P, P2H2P`
      } else {
    ...
    businessErrors.routingType = `Error while checking routing type in /${constants.ON_CONFIRM}: ${error.message}`
  }
  ...
  if (schemaValidation === true) {
    if (hasSchemaErrors) {
      return { success: false, schemaErrors }
    } else {
      return { success: true, error: false }
    }
  }

  if (hasSchemaErrors || hasBusinessErrors) {
    return { success: false, schemaErrors, businessErrors }
  } else {
    return { success: true, error: false }
  }
}
