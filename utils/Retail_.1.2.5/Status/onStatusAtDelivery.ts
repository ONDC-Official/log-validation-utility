...
export const checkOnStatusAtDelivery = (data: any, state: string, msgIdSet: any, fulfillmentsItemsSet: any) => {
  ...
  try {
    ...
    const at_delivery_message_id: string = context.message_id

    setValue(`at_delivery_message_id`, at_delivery_message_id)

    try {
      logger.info(
        `Comparing message_id for unsolicited calls for ${constants.ON_STATUS}.packed and ${constants.ON_STATUS}.at_delivery`,
      )

      if (packed_message_id === at_delivery_message_id) {
        onStatusObj['invalid_message_id'] =
          `Message_id cannot be same for ${constants.ON_STATUS}.packed and ${constants.ON_STATUS}.at_delivery`
      }
    } catch (error: any) {
      logger.error(
        `Error while comparing message_id for ${constants.ON_STATUS}.packed and ${constants.ON_STATUS}.at_delivery`,
      )
    }
