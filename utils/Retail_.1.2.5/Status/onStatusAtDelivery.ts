...
export const checkOnStatusAtDelivery = (data: any, _state: string, msgIdSet: any, _fulfillmentsItemsSet: any, flow?: string) => {
  ...
  try {
    if (!data || isObjectEmpty(data)) {
      return { [ApiSequence.ON_STATUS_AT_DELIVERY]: 'JSON cannot be empty' }
    }
    ...
    try {
      ...
      if (msgIdSet.has(context.message_id)) {
        onStatusObj[`${ApiSequence.ON_STATUS_AT_DELIVERY}_msgId`] = `Message id should not be same with previous calls`
      }
    ...
    try {
      ...
      if (DeliveryFulfillment) {
        ...
        if (currentFulfillment) {
          ...
          setValue('DeliveryFulfillmentAction', ApiSequence.ON_STATUS_AT_DELIVERY)
        }
      }
    } catch (error: any) {
