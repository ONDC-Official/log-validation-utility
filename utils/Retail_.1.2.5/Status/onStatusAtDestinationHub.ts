...
export const checkOnStatusAtDestinationHub = (data: any, _state: string, msgIdSet: any, _fulfillmentsItemsSet: any, flow?: string) => {
  const onStatusObj: any = {}
  const EXPECTED_STATE = 'At-destination-hub'
  
  try {
    if (!data || isObjectEmpty(data)) {
      return { [ApiSequence.ON_STATUS_AT_DESTINATION_HUB]: 'JSON cannot be empty' }
    }
    ...
    try {
      ...
      if (msgIdSet.has(context.message_id)) {
        onStatusObj[`${ApiSequence.ON_STATUS_AT_DESTINATION_HUB}_msgId`] = `Message id should not be same with previous calls`
      }
    ...
    setValue(`${ApiSequence.ON_STATUS_AT_DESTINATION_HUB}`, data)
    ...
    try {
      ...
      if (DeliveryFulfillment) {
        ...
        if (currentFulfillment) {
          ...
          setValue('DeliveryFulfillmentAction', ApiSequence.ON_STATUS_AT_DESTINATION_HUB)
        }
      }
    } catch (error: any) {
