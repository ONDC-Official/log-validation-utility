...
export const checkOnStatusOutForDelivery = (data: any, state: string, msgIdSet: any, fulfillmentsItemsSet: any) => {
  ...
  try {
    ...
    if(state === ApiSequence.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY){
      try {
      ...
      try {
        ...
        if (deliveryFulfillment.length === 0) {
          onStatusObj['rplcmnt-fulfillment'] =
            `Fulfillment with id :${replacementFulfillment.id} not found in ${ApiSequence.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY} fulfillments`
          return onStatusObj
        }
      ...
      try {
        ...
        logger.error(`!!Error while checking order updated timestamp in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}, ${error.stack}`)
      }
  
      try {
        if (!_.isEqual(getValue(`cnfrmTmpstmp`), on_status.created_at)) {
          onStatusObj.tmpstmp = `Created At timestamp for /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY} should be equal to context timestamp at ${constants.CONFIRM}`
        }
      } catch (error: any) {
        logger.error(
          `!!Error occurred while comparing order created at for /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}, ${error.stack}`,
        )
      }
      ...
      try {
        ...
        if (on_status.state != 'Completed') {
          onStatusObj.ordrState = `order/state should be "Completed" for /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}`
        }
      } catch (error: any) {
        logger.error(`!!Error while checking order state in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY} Error: ${error.stack}`)
      }
  
      try {
        logger.info(`Checking Out_for_delivery timestamp in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}`)
          ...
          if (ffState === constants.ORDER_OUT_FOR_DELIVERY) {
            ...
            if (!out_for_delivery_time) {
              ...
              try {
                ...
                logger.error(
                  `!!Error while checking Out_for_delivery time matching with context timestamp in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}`,
                  error,
                )
              }
              ...
              try {
                ...
                logger.error(
                  `!!Error while checking order/updated_at timestamp in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}`,
                  error,
                )
              }
            }
        ...
        logger.info(
          `Error while checking out for delivery timestamp in /${constants.REPLACEMENT_ON_STATUS_OUT_FOR_DELIVERY}.json Error: ${error.stack}`,
        )
      }
    }
    if(state === ApiSequence.ON_STATUS_OUT_FOR_DELIVERY){
      try {
        logger.info(`Comparing order Id in /${constants.ON_CONFIRM} and /${constants.ON_STATUS}_${state}`)
        if (on_status.id != getValue('cnfrmOrdrId')) {
          logger.info(`Order id (/${constants.ON_STATUS}_${state}) mismatches with /${constants.CONFIRM})`)
