...
export const checkOnStatusPending = (
 data: any, 
 state: string, 
 msgIdSet: any, 
 ...
 try {
 // 1. Basic validations (always run)
 const { message, context }: any = data
 if (!message || !context || isObjectEmpty(message)) {
 return { missingFields: '/context, /message, is missing or empty' }
 }

 // 2. Schema validation (conditional)
 ...
 // 3. Context validation (always run)
 ...
 businessErrors[`${ApiSequence.ON_STATUS_PENDING}_msgId`] = `Message id should not be same with previous calls`
 }
 msgIdSet.add(context.message_id)
 if (!stateless) setValue('pending_message_id', context.message_id)
 } catch (error: any) {
 logger.error(`!!Error while checking message id for /${constants.ON_STATUS_PENDING}, ${error.stack}`)
 }

 // 5. Early return for stateless mode
 if (stateless) {
 ...
 // 6. Stateful validations (only when stateless = false)
 const flow = getValue('flow')
 const replaceValue = getValue('replaceValue)')

 // Domain consistency check (requires previous state)
 ...
 if (!res.valid) {
 Object.assign(businessErrors, res.ERRORS)
 }
 } catch (error: any) {
 ...
 businessErrors.city = `City code mismatch in /${constants.SEARCH} and /${constants.ON_STATUS}`
 }
 } catch (error: any) {
 ...
 businessErrors["rplcmnt"] = `Fulfillment for replacement is required.`
 // Return logic for early exit
 // const hasSchema = Object.keys(schemaErrors).length > 0
 // const hasBusiness = Object.keys(businessErrors).length > 0
 if (schemaValidation !== undefined) {
 return { schemaErrors, businessErrors }
 ...
 if(!orderState){
 businessErrors["rplcmntOrderState"] = `Mismatch ${ApiSequence.REPLACEMENT_ON_STATUS_PENDING} and ${ApiSequence.ON_UPDATE_REPLACEMENT} and order state should be completed`
 }
 ...
 businessErrors.onStatusOdrId = `Order id in /${constants.CONFIRM} and /${constants.REPLACEMENT_ON_STATUS_PENDING} do not match`
 }
 } catch (error) {
 logger.info(
 ...
 businessErrors['ffId'] = `Fulfillment Id must be present`
 }
 ...
 businessErrors['ffTracking'] = `Tracking must be present for fulfillment ID: ${ff.id} in boolean form`
 }
 }
 }
 })
 ...
 businessErrors["rplcmnt-fulfillment"] = `Fulfillment with id :${replacementFulfillment.id} not found in ${ApiSequence.REPLACEMENT_ON_STATUS_PENDING} fulfillments`
 // Return logic
 // const hasSchema = Object.keys(schemaErrors).length > 0
 // const hasBusiness = Object.keys(businessErrors).length > 0
 if (schemaValidation !== undefined) {
 ...
 businessErrors[`message.order.fulfillments[${deliveryFulfillment.id}]`] =
 ...
 businessErrors[key] = `default fulfillments state is missing in /${constants.ON_UPDATE_REPLACEMENT}`
 }

 if (!deliveryFulfillment.start || !deliveryFulfillment.end) {
 businessErrors.ffstartend = `fulfillments start and end locations are mandatory for fulfillment id: ${deliveryFulfillment.id}`
 }

 try {
 if (!compareCoordinates(deliveryFulfillment.start.location.gps, getValue('providerGps'))) {
 businessErrors.sellerGpsErr = `store gps location /fulfillments/:${deliveryFulfillment.id}/start/location/gps can't change`
 }
 } catch (error: any) {
 ...
 businessErrors.sellerNameErr = `Invalid store name inside fulfillments in /${constants.ON_UPDATE_REPLACEMENT}`
 } else if (!_.isEqual(deliveryFulfillment.start.location.descriptor.name, getValue('providerName'))) {
 businessErrors.sellerNameErr = `store name /fulfillments/start/location/descriptor/name can't change with fulfillment id: ${deliveryFulfillment.id}`
 }
 } catch (error: any) {
 ...
 businessErrors.buyerGpsErr = `fulfillments.end.location gps with id ${deliveryFulfillment.id} is not matching with gps in /on_conifrm`
 }

 if (!_.isEqual(deliveryFulfillment.end.location.address.area_code, getValue('buyerAddr'))) {
 businessErrors.gpsErr = `fulfillments.end.location.address.area_code with id ${deliveryFulfillment.id} is not matching with area_code in /on_confirm`
 }
 } catch (error: any) {
 logger.error(
 ...
 businessErrors.onStatusOdrId = `Order id in /${constants.CONFIRM} and /${constants.ON_STATUS}_${state} do not match`
 }
 } catch (error) {
 logger.info(
 ...
 businessErrors[key] = `${fulfillmentRangeerrors[i]}`
 i++
 }
 }
 }
 ...
 businessErrors.tmpstmp = `Created At timestamp for /${constants.ON_STATUS}_${state} should be equal to context timestamp at ${constants.CONFIRM}`
 }
 } catch (error: any) {
 ...
 businessErrors.tmpstmp1 = `Timestamp for /${constants.ON_CONFIRM} api cannot be greater than or equal to /${constants.ON_STATUS}_${state} api`
 }
 ...
 businessErrors.tmpstmp2 = `order.updated_at timestamp should be less than or eqaul to context timestamp for /${constants.ON_STATUS}_${state} api`
 }
 } catch (error: any) {
 logger.error(
 ...
 businessErrors[key] = `missingFulfillments is mandatory for ${ApiSequence.ON_STATUS_PENDING}`
 }
 else {
 const deliveryObjArr = _.filter(fulfillments, { type: "Delivery" })
 if (!deliveryObjArr.length) {
 businessErrors[`message/order.fulfillments/`] = `Delivery fullfillment must be present in ${ApiSequence.ON_STATUS_PENDING}`
 }
 else {
 ...
 if (!expectedCreds) {
 businessErrors['MissingCreds'] =
 `creds must be present in /${constants.ON_CANCEL} same as in /${constants.ON_SEARCH}`
 }
 if (flow === FLOW.FLOW017) {
 if (!expectedCreds) {
 businessErrors['MissingCreds'] = `creds must be present in /${constants.ON_STATUS_PENDING} `
 ...
 businessErrors['missingFulfillments'] =
 ...
 businessErrors['providerIdMismatch'] =
 ...
 if (!routingType) {
 businessErrors.routingType = `Routing type tag is mandatory in delivery fulfillment when order.state was 'Created' in /${constants.ON_CONFIRM}`
 ...
 businessErrors.routingTypeMismatch = `Routing type mismatch: '${routingType}' in /${constants.ON_STATUS_PENDING} does not match '${storedRoutingType}' from /${constants.ON_CONFIRM}`
 ...
 const flow = !stateless ? getValue('flow') : 'unknown'
