    ...
    if(state === ApiSequence.REPLACEMENT_ON_STATUS_PICKED){
      try {
        ...
        if (on_status.id != getValue('cnfrmOrdrId')) {
          logger.info(`Order id (/${constants.REPLACEMENT_ON_STATUS_PICKED}) mismatches with /${constants.CONFIRM})`)
          onStatusObj.onStatusOdrId = `Order id in /${constants.CONFIRM} and /${constants.REPLACEMENT_ON_STATUS_PICKED} do not match`
        }
      } catch (error) {
        logger.info(
          `!!Error while comparing order id in /${constants.REPLACEMENT_ON_STATUS_PICKED} and /${constants.CONFIRM}`,
          error,
        )
      }
      try {
        logger.info(`Comparing timestamp of /${constants.ON_STATUS}_picked and /${constants.REPLACEMENT_ON_STATUS_PICKED} API`)
        if (_.gte(getValue('tmpstmp'), context.timestamp)) {
          onStatusObj.inVldTmstmp = `Timestamp in previous /${constants.ON_STATUS} api cannot be greater than or equal to /${constants.REPLACEMENT_ON_STATUS_PICKED} api`
        }
  
        setValue('tmpstmp', context.timestamp)
      } catch (error: any) {
        logger.error(`!!Error occurred while comparing timestamp for /${constants.REPLACEMENT_ON_STATUS_PICKED}, ${error.stack}`)
      }
      try {
        logger.info(`Comparing timestamp of /${constants.ON_CONFIRM} and /${constants.REPLACEMENT_ON_STATUS_PICKED} API`)
        if (_.gte(getValue('onCnfrmtmpstmp'), context.timestamp)) {
          onStatusObj.tmpstmp1 = `Timestamp for /${constants.ON_CONFIRM} api cannot be greater than or equal to /${constants.REPLACEMENT_ON_STATUS_PICKED} api`
        }
      } catch (error: any) {
        logger.error(`!!Error occurred while comparing timestamp for /${constants.REPLACEMENT_ON_STATUS_PICKED}, ${error.stack}`)
      }
      ...
      try {
        // For Delivery Object
        const DELobj = on_status.fulfillments.filter(
          (fulfillment: any) => fulfillment.id === replacementFulfillment.id,
        )
        if (!DELobj.length) {
          logger.error(`Delivery object is mandatory for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`)
          const key = `missingDelivery`
          onStatusObj[key] = `Delivery object is mandatory for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`
        } else {
          const deliveryObj = DELobj[0]
          if (!deliveryObj.tags) {
            const key = `missingTags`
            onStatusObj[key] = `Tags are mandatory in Delivery Fulfillment for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`
          }
          else {
            ...
            if (!routingTagArr.length) {
              ...
              onStatusObj[key] = `RoutingTag object is mandatory in Tags of Delivery Object for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`
            }
            else {
              ...
              if (!routingTagList) {
                ...
                onStatusObj[key] = `RoutingTagList is mandatory in RoutingTag of Delivery Object for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`
              }
              else {
                ...
                if (!routingTagTypeArr.length) {
                  ...
                  onStatusObj[key] = `RoutingTagListType object is mandatory in RoutingTag/List of Delivery Object for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED}`
                }
                else {
                  ...
                  if (!ROUTING_ENUMS.includes(routingTagType.value)) {
                    ...
                    onStatusObj[key] = `RoutingTagListType Value is mandatory in RoutingTag of Delivery Object for ${ApiSequence.REPLACEMENT_ON_STATUS_PICKED} and should be equal to 'P2P' or 'P2H2P'`;
                  }
                }
              }
            }
      ...
      try {
        logger.info(`Comparing order.updated_at and context timestamp for /${constants.REPLACEMENT_ON_STATUS_PICKED} API`)
        if (!_.isEqual(on_status.created_at, getValue(`cnfrmTmpstmp`))) {
          onStatusObj.tmpstmp = `Created At timestamp for /${constants.REPLACEMENT_ON_STATUS_PICKED} should be equal to context timestamp at ${constants.CONFIRM}`
        }
  
        if (!areTimestampsLessThanOrEqualTo(on_status.updated_at, contextTime)) {
          onStatusObj.tmpstmp2 = ` order.updated_at timestamp should be less than or eqaul to  context timestamp for /${constants.REPLACEMENT_ON_STATUS_PICKED} api`
        }
      } catch (error: any) {
        logger.error(
          `!!Error occurred while comparing order updated at for /${constants.REPLACEMENT_ON_STATUS_PICKED}, ${error.stack}`,
        )
      }
  
      try {
        logger.info(`Checking order state in /${constants.REPLACEMENT_ON_STATUS_PICKED}`)
        if (on_status.state != 'Completed') {
          onStatusObj.ordrState = `order/state should be "REPLACEMENT_ON_STATUS_PICKED" for /${constants.REPLACEMENT_ON_STATUS_PICKED}`
        }
      } catch (error: any) {
        logger.error(`!!Error while checking order state in /${constants.REPLACEMENT_ON_STATUS_PICKED} Error: ${error.stack}`)
      }
  
      try {
        logger.info(`Checking pickup timestamp in /${constants.REPLACEMENT_ON_STATUS_PICKED}`)
        ...
        // while (i < noOfFulfillments) {
          const fulfillment = on_status.fulfillments.find((fulfillment:any)=> fulfillment.id === replacementFulfillment.id)
          const ffState = fulfillment.state.descriptor.code
  
          //type should be Delivery
          // if (fulfillment.id != replacementFulfillment.id) {
          //   i++
          //   continue
          // }
          ...
          if (ffState === constants.ORDER_PICKED) {
            ...
            if (!pickUpTime) {
              ...
              try {
                ...
                logger.error(
                  `!!Error while checking pickup time matching with context timestamp in /${constants.REPLACEMENT_ON_STATUS_PICKED}`,
                  error,
                )
              }
              ...
              try {
                ...
                logger.error(
                  `!!Error while checking order/updated_at timestamp in /${constants.REPLACEMENT_ON_STATUS_PICKED}`,
                  error,
                )
              }
            }
        ...
        try {
          ...
          const deliveryFulfillment = on_status.fulfillments.filter((fulfillment: any) => fulfillment.id === replacementFulfillment.id)
        ...
        if (!orderPicked) {
          onStatusObj.noOrdrPicked = `fulfillments/state should be ${constants.ORDER_PICKED} for /${constants.REPLACEMENT_ON_STATUS_PICKED}`
        }
      } catch (error: any) {
        logger.info(
          `Error while checking pickup timestamp in /${constants.REPLACEMENT_ON_STATUS_PICKED}.json Error: ${error.stack}`,
        )
      }
    }
    if (state === "picked") {
