...
export const checkOnStatusPickupFailed = (data: any, _state: string, msgIdSet: any, _fulfillmentsItemsSet: any, flow?: string) => {
  const onStatusObj: any = {}
  const EXPECTED_STATE = 'Pickup-failed'
  
  try {
    if (!data || isObjectEmpty(data)) {
      return { [ApiSequence.ON_STATUS_PICKUP_FAILED]: 'JSON cannot be empty' }
    }
    ...
    try {
      ...
      if (msgIdSet.has(context.message_id)) {
        onStatusObj[`${ApiSequence.ON_STATUS_PICKUP_FAILED}_msgId`] = `Message id should not be same with previous calls`
      }
    ...
    setValue(`${ApiSequence.ON_STATUS_PICKUP_FAILED}`, data)
    ...
    try {
      ...
      if (DeliveryFulfillment) {
        ...
        if (currentFulfillment) {
          ...
          setValue('DeliveryFulfillmentAction', ApiSequence.ON_STATUS_PICKUP_FAILED)
        }
      }
    } catch (error: any) {
