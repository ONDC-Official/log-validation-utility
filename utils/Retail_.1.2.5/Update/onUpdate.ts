  ...
  try {
    ...
    if (stateless) {
      if (schemaValidation === true) {
        return Object.keys(schemaErrors).length ? { schemaErrors } : false;
      }
      if (schemaValidation === false) {
        return Object.keys(onupdtObj).length ? { businessErrors: onupdtObj } : false;
      }
    ...
    return { [apiSeq]: `Some error occurred while checking /${apiSeq} API` }
  }
}
