...
export const checkOnConfirm = (data: any, msgIdSet: any, version: any) => {
  ...
  if (!data || isObjectEmpty(data)) {
    return { [TRV13ApiSequence.ON_CONFIRM]: 'JSON cannot be empty' }
  }
  ...
  const schemaValidation = validateSchema('TRV13', TRV13ApiSequence.ON_CONFIRM, data)
  const contextRes: any = validateContext(context, msgIdSet, TRV13ApiSequence.CONFIRM, TRV13ApiSequence.ON_CONFIRM)
  setValue(`${TRV13ApiSequence.ON_CONFIRM}_message`, message)
  const errorObj: any = {}
  ...
  try {
    ...
    // Items validation
    if (!onConfirmOrder?.items || !Array.isArray(onConfirmOrder.items) || onConfirmOrder.items.length === 0) {
      errorObj.items = 'items array is missing or empty'
    } else {
      onConfirmOrder.items.forEach((item: any, index: number) => {
        // Item ID validation
        if (!item?.id) {
    ...
    // Quote validation
    if (!onConfirmOrder?.quote) {
      errorObj.quote = 'quote is missing in order'
    } else {
      const quote = onConfirmOrder.quote
    ...
    // Payments validation
    if (!onConfirmOrder?.payments || !Array.isArray(onConfirmOrder.payments) || onConfirmOrder.payments.length === 0) {
      errorObj.payments = 'payments array is missing or empty'
    } else {
      onConfirmOrder.payments.forEach((payment: any, index: number) => {
    ...
    // Billing validation
    if (!onConfirmOrder?.billing) {
      errorObj.billing = 'billing is missing in order'
    } else {
      const billing = onConfirmOrder.billing
    ...
    if (!onConfirmOrder?.fulfillments || !Array.isArray(onConfirmOrder.fulfillments) || onConfirmOrder.fulfillments.length === 0) {
      errorObj.fulfillments = 'fulfillments array is missing or empty'
    } else {
      onConfirmOrder.fulfillments.forEach((fulfillment: any, index: number) => {
    ...
    // Tags validation
    if (onConfirmOrder?.tags) {
      if (!Array.isArray(onConfirmOrder.tags)) {
        errorObj.tags = 'order.tags should be an array'
      } else {
        onConfirmOrder.tags.forEach((tag: any, index: number) => {
          if (!tag?.descriptor) {
    ...
    if (!onConfirmOrder?.updated_at) {
      errorObj.updated_at = 'updated_at is missing'
    } else {
      const updatedDate = new Date(onConfirmOrder.updated_at)
    ...
    logger.error(`!!Error while checking on_confirm info in /${TRV13ApiSequence.ON_CONFIRM}, ${error.stack}`)
