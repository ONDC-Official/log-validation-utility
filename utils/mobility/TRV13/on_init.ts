...
export const checkOnInit = (data: any, msgIdSet: any, version: any) => {
  ...
  if (!data || isObjectEmpty(data)) {
    return { [TRV13ApiSequence.ON_INIT]: 'JSON cannot be empty' }
  }
  ...
  const schemaValidation = validateSchema('TRV13', TRV13ApiSequence.ON_INIT, data)
  const contextRes: any = validateContext(context, msgIdSet, TRV13ApiSequence.INIT, TRV13ApiSequence.ON_INIT)
  setValue(`${TRV13ApiSequence.ON_INIT}_message`, message)
  const errorObj: any = {}
  ...
  const onInitOrder: any = message.order

  try {
    // Provider validation
    if (!onInitOrder?.provider) {
      errorObj.provider = 'provider is missing in order'
    } else {
      const provider = onInitOrder.provider
      if (!provider?.id) {
    ...
    // Items validation
    if (!onInitOrder?.items || !Array.isArray(onInitOrder.items) || onInitOrder.items.length === 0) {
      errorObj.items = 'items array is missing or empty'
    } else {
      onInitOrder.items.forEach((item: any, index: number) => {
        // Item ID validation
        if (!item?.id) {
    ...
    // Quote validation
    if (!onInitOrder?.quote) {
      errorObj.quote = 'quote is missing in order'
    } else {
      const quote = onInitOrder.quote
    ...
    // Payments validation
    if (!onInitOrder?.payments || !Array.isArray(onInitOrder.payments) || onInitOrder.payments.length === 0) {
      errorObj.payments = 'payments array is missing or empty'
    } else {
      onInitOrder.payments.forEach((payment: any, index: number) => {
    ...
    // Billing validation
    if (!onInitOrder?.billing) {
      errorObj.billing = 'billing is missing in order'
    } else {
      const billing = onInitOrder.billing
    ...
    if (!onInitOrder?.fulfillments || !Array.isArray(onInitOrder.fulfillments) || onInitOrder.fulfillments.length === 0) {
      errorObj.fulfillments = 'fulfillments array is missing or empty'
    } else {
      onInitOrder.fulfillments.forEach((fulfillment: any, index: number) => {
    ...
    // Tags validation
    if (onInitOrder?.tags) {
      if (!Array.isArray(onInitOrder.tags)) {
        errorObj.tags = 'order.tags should be an array'
      } else {
        onInitOrder.tags.forEach((tag: any, index: number) => {
          if (!tag?.descriptor) {
    ...
    logger.error(`!!Error while checking on_init info in /${TRV13ApiSequence.ON_INIT}, ${error.stack}`)
