...
import constants, {TRV13ApiSequence } from '../../../constants'
...
export const checkOnSearch = (data: any, msgIdSet: any, version: any) => {
  ...
  if (!data || isObjectEmpty(data)) {
    return { [TRV13ApiSequence.ON_SEARCH]: 'JSON cannot be empty' }
  }
  ...
  const schemaValidation = validateSchema('TRV13', constants.ON_SEARCH, data)
  const contextRes: any = validateContext(context, msgIdSet, constants.SEARCH, constants.ON_SEARCH)
  setValue(`${TRV13ApiSequence.ON_SEARCH}_message`, message)
  const errorObj: any = {}
  ...
  try {
    // Catalog descriptor validation
    if (!onSearchCatalog?.descriptor) {
      errorObj.catalog_descriptor = 'catalog.descriptor is missing'
    } else {
      if (!onSearchCatalog.descriptor?.name) {
        errorObj.catalog_descriptor_name = 'catalog.descriptor.name is missing'
      }
      if (!onSearchCatalog.descriptor?.code) {
        errorObj.catalog_descriptor_code = 'catalog.descriptor.code is missing'
      }
      if (!onSearchCatalog.descriptor?.short_desc) {
        errorObj.catalog_descriptor_short_desc = 'catalog.descriptor.short_desc is missing'
      }
      if (!onSearchCatalog.descriptor?.long_desc) {
        errorObj.catalog_descriptor_long_desc = 'catalog.descriptor.long_desc is missing'
      }
      if (!onSearchCatalog.descriptor?.images || onSearchCatalog.descriptor.images.length === 0) {
        errorObj.catalog_descriptor_images = 'catalog.descriptor.images array is missing or empty'
      } else {
        onSearchCatalog.descriptor.images.forEach((image: any, i: number) => {
          if (!image?.url) {
            errorObj[`catalog_descriptor_image_${i}_url`] = `image.url is missing at index ${i}`
          }
        })
      }
    }
    ...
    while (i < len) {
      ...
      if (!bppPrvdrs[i]?.descriptor) {
        errorObj[`provider_${i}_descriptor`] = `provider.descriptor is missing at index ${i}`
      } else {
        if (!bppPrvdrs[i].descriptor?.name) {
          errorObj[`provider_${i}_descriptor_name`] = `provider.descriptor.name is missing at index ${i}`
        }
        if (!bppPrvdrs[i].descriptor?.code) {
          errorObj[`provider_${i}_descriptor_code`] = `provider.descriptor.code is missing at index ${i}`
        }
        if (!bppPrvdrs[i].descriptor?.short_desc) {
          errorObj[`provider_${i}_descriptor_short_desc`] = `provider.descriptor.short_desc is missing at index ${i}`
        }
        if (!bppPrvdrs[i].descriptor?.long_desc) {
          errorObj[`provider_${i}_descriptor_long_desc`] = `provider.descriptor.long_desc is missing at index ${i}`
        }
        if (!bppPrvdrs[i].descriptor?.images || bppPrvdrs[i].descriptor.images.length === 0) {
          errorObj[`provider_${i}_descriptor_images`] = `provider.descriptor.images array is missing or empty at index ${i}`
        } else {
          bppPrvdrs[i].descriptor.images.forEach((image: any, j: number) => {
            if (!image?.url) {
              errorObj[`provider_${i}_descriptor_image_${j}_url`] = `image.url is missing at index ${j}`
            }
          })
        }
      }

      // Provider locations validation
      if (!bppPrvdrs[i]?.locations || bppPrvdrs[i].locations.length === 0) {
        errorObj[`provider_${i}_locations`] = `provider.locations array is missing or empty at index ${i}`
      } else {
        bppPrvdrs[i].locations.forEach((location: any, j: number) => {
          if (!location?.id) {
            errorObj[`provider_${i}_location_${j}_id`] = `location.id is missing at index ${j}`
          } else {
            locationIds.add(location.id)
          }
          if (!location?.gps) {
            errorObj[`provider_${i}_location_${j}_gps`] = `location.gps is missing at index ${j}`
          }
          if (!location?.address) {
            errorObj[`provider_${i}_location_${j}_address`] = `location.address is missing at index ${j}`
          }
          if (!location?.city?.name) {
            errorObj[`provider_${i}_location_${j}_city_name`] = `location.city.name is missing at index ${j}`
          }
          if (!location?.state?.name) {
            errorObj[`provider_${i}_location_${j}_state_name`] = `location.state.name is missing at index ${j}`
          }
          if (!location?.area_code) {
            errorObj[`provider_${i}_location_${j}_area_code`] = `location.area_code is missing at index ${j}`
          }
          if (!location?.rating) {
            errorObj[`provider_${i}_location_${j}_rating`] = `location.rating is missing at index ${j}`
          }
        })
      }
      ...
      if (!payments || payments.length === 0) {
        errorObj[`provider_${i}_payments`] = `payments array is missing or empty for provider ${i}`
      } else {
        payments.forEach((payment: any, j: number) => {
          if (!payment?.id) {
            errorObj[`provider_${i}_payment_${j}_id`] = `payment.id is missing at index ${j}`
          } else if (paymentIds.has(payment.id)) {
            errorObj[`provider_${i}_payment_${j}_id`] = `duplicate payment id: ${payment.id}`
          } else {
            paymentIds.add(payment.id)
          }

          if (!payment?.type) {
            errorObj[`provider_${i}_payment_${j}_type`] = `payment.type is missing at index ${j}`
          } else {
            const validTypes = ['PRE-ORDER', 'ON-FULFILLMENT', 'PART-PAYMENT']
            if (!validTypes.includes(payment.type)) {
              errorObj[`provider_${i}_payment_${j}_type`] = `payment.type should be one of ${validTypes.join(', ')}`
            }
          }
        })
      }
      ...
      if (!items || items.length === 0) {
        ...
        items.forEach((item: any, j: number) => {
          ...
          // Item location_ids validation
          if (!item?.location_ids || item.location_ids.length === 0) {
            errorObj[`provider_${i}_item_${j}_location_ids`] = `item.location_ids array is missing or empty at index ${j}`
          } else {
            item.location_ids.forEach((locId: string) => {
              if (!locationIds.has(locId)) {
                errorObj[`provider_${i}_item_${j}_location_ids`] = `invalid location_id: ${locId} at index ${j}`
              }
            })
          }
          ...
          // Item recommended flag validation
          if (item?.recommended !== undefined && typeof item.recommended !== 'boolean') {
            errorObj[`provider_${i}_item_${j}_recommended`] = `item.recommended should be a boolean value at index ${j}`
          }
        })
      }
    ...
    logger.error(`!!Error while checking Providers info in /${TRV13ApiSequence.ON_SEARCH}, ${error.stack}`)
