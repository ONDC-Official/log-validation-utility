...
export const checkOnStatus = (data: any, msgIdSet: any, version: any) => {
  ...
  if (!data || isObjectEmpty(data)) {
    return { [TRV13ApiSequence.ON_STATUS]: 'JSON cannot be empty' }
  }
  ...
  const schemaValidation = validateSchema('TRV13', TRV13ApiSequence.ON_STATUS, data)
  const contextRes: any = validateContext(context, msgIdSet, TRV13ApiSequence.STATUS, TRV13ApiSequence.ON_STATUS)
  setValue(`${TRV13ApiSequence.ON_STATUS}_message`, message)
  const errorObj: any = {}
  ...
  const onStatusOrder: any = message.order
  ...
  try {
    ...
    // Items validation
    if (!onStatusOrder?.items || !Array.isArray(onStatusOrder.items) || onStatusOrder.items.length === 0) {
      errorObj.items = 'items array is missing or empty'
    } else {
      onStatusOrder.items.forEach((item: any, index: number) => {
        // Item ID validation
        if (!item?.id) {
    ...
    // Quote validation
    if (!onStatusOrder?.quote) {
      errorObj.quote = 'quote is missing in order'
    } else {
      const quote = onStatusOrder.quote
    ...
    // Payments validation
    if (!onStatusOrder?.payments || !Array.isArray(onStatusOrder.payments) || onStatusOrder.payments.length === 0) {
      errorObj.payments = 'payments array is missing or empty'
    } else {
      onStatusOrder.payments.forEach((payment: any, index: number) => {
    ...
    // Billing validation
    if (!onStatusOrder?.billing) {
      errorObj.billing = 'billing is missing in order'
    } else {
      const billing = onStatusOrder.billing
    ...
    if (!onStatusOrder?.fulfillments || !Array.isArray(onStatusOrder.fulfillments) || onStatusOrder.fulfillments.length === 0) {
      errorObj.fulfillments = 'fulfillments array is missing or empty'
    } else {
      onStatusOrder.fulfillments.forEach((fulfillment: any, index: number) => {
    ...
    // Tags validation
    if (onStatusOrder?.tags) {
      if (!Array.isArray(onStatusOrder.tags)) {
        errorObj.tags = 'order.tags should be an array'
      } else {
        onStatusOrder.tags.forEach((tag: any, index: number) => {
          if (!tag?.descriptor) {
    ...
    if (onStatusOrder?.documents) {
      if (!Array.isArray(onStatusOrder.documents)) {
        errorObj.documents = 'order.documents should be an array'
      } else {
        onStatusOrder.documents.forEach((doc: any, index: number) => {
          if (!doc?.descriptor) {
    ...
    if (!onStatusOrder?.updated_at) {
      errorObj.updated_at = 'updated_at is missing'
    } else {
      const updatedDate = new Date(onStatusOrder.updated_at)
    ...
    logger.error(`!!Error while checking on_status info in /${TRV13ApiSequence.ON_STATUS}, ${error.stack}`)
