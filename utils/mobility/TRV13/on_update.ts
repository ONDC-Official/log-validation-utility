...
export const checkOnUpdate = (data: any, msgIdSet: any, version: any) => {
  ...
  if (!data || isObjectEmpty(data)) {
    return { [TRV13ApiSequence.ON_UPDATE]: 'JSON cannot be empty' }
  }
  ...
  const schemaValidation = validateSchema('TRV13', TRV13ApiSequence.ON_UPDATE, data)
  const contextRes: any = validateContext(context, msgIdSet, TRV13ApiSequence.UPDATE, TRV13ApiSequence.ON_UPDATE)
  setValue(`${TRV13ApiSequence.ON_UPDATE}_message`, message)
  const errorObj: any = {}
  ...
  const onUpdateOrder: any = message.order
  ...
  try {
    ...
    if (!onUpdateOrder?.status) {
      errorObj.order_status = 'order.status is missing'
    } else {
      const validStatuses = ['ACTIVE', 'CANCELLED', 'COMPLETE']
      if (!validStatuses.includes(onUpdateOrder.status)) {
        errorObj.order_status = `order.status should be one of ${validStatuses.join(', ')}`
      }
    }

    // Provider validation
    if (!onUpdateOrder?.provider) {
      errorObj.provider = 'provider is missing in order'
    } else {
      const provider = onUpdateOrder.provider
      if (!provider?.id) {
    ...
    // Items validation
    if (!onUpdateOrder?.items || !Array.isArray(onUpdateOrder.items) || onUpdateOrder.items.length === 0) {
      errorObj.items = 'items array is missing or empty'
    } else {
      onUpdateOrder.items.forEach((item: any, index: number) => {
        // Item ID validation
        if (!item?.id) {
    ...
    // Quote validation
    if (!onUpdateOrder?.quote) {
      errorObj.quote = 'quote is missing in order'
    } else {
      const quote = onUpdateOrder.quote
    ...
    // Payments validation
    if (!onUpdateOrder?.payments || !Array.isArray(onUpdateOrder.payments) || onUpdateOrder.payments.length === 0) {
      errorObj.payments = 'payments array is missing or empty'
    } else {
      onUpdateOrder.payments.forEach((payment: any, index: number) => {
        ...
        if (payment?.tags) {
          if (!Array.isArray(payment.tags)) {
            errorObj[`payment_${index}_tags`] = `payment.tags should be an array at index ${index}`
          } else {
        ...
        if (payment?.time) {
          if (!payment.time?.timestamp) {
            errorObj[`payment_${index}_time_timestamp`] = `payment.time.timestamp is missing at index ${index}`
          } else {
            const timestamp = new Date(payment.time.timestamp)
            if (isNaN(timestamp.getTime())) {
              errorObj[`payment_${index}_time_timestamp`] = `payment.time.timestamp should be a valid ISO date string at index ${index}`
            }
          }
        }
      })
    ...
    // Billing validation
    if (!onUpdateOrder?.billing) {
      errorObj.billing = 'billing is missing in order'
    } else {
      const billing = onUpdateOrder.billing
    ...
    if (!onUpdateOrder?.fulfillments || !Array.isArray(onUpdateOrder.fulfillments) || onUpdateOrder.fulfillments.length === 0) {
      errorObj.fulfillments = 'fulfillments array is missing or empty'
    } else {
      onUpdateOrder.fulfillments.forEach((fulfillment: any, index: number) => {
    ...
    // Tags validation
    if (onUpdateOrder?.tags) {
      if (!Array.isArray(onUpdateOrder.tags)) {
        errorObj.tags = 'order.tags should be an array'
      } else {
        onUpdateOrder.tags.forEach((tag: any, index: number) => {
          if (!tag?.descriptor) {
    ...
    if (onUpdateOrder?.documents) {
      if (!Array.isArray(onUpdateOrder.documents)) {
        errorObj.documents = 'order.documents should be an array'
      } else {
        onUpdateOrder.documents.forEach((doc: any, index: number) => {
          if (!doc?.descriptor) {
            errorObj[`document_${index}_descriptor`] = `document.descriptor is missing at index ${index}`
          } else {
            if (!doc.descriptor?.code) {
              errorObj[`document_${index}_descriptor_code`] = `document.descriptor.code is missing at index ${index}`
            } else {
              const validCodes = ['BOOKING_CONFIRMATION', 'INVOICE']
              if (!validCodes.includes(doc.descriptor.code)) {
                errorObj[`document_${index}_descriptor_code`] = `document.descriptor.code should be one of ${validCodes.join(', ')} at index ${index}`
              }
            }
          }

          if (!doc?.url) {
            errorObj[`document_${index}_url`] = `document.url is missing at index ${index}`
          } else {
            const urlRegex = /^https?:\/\/.+/
            if (!urlRegex.test(doc.url)) {
              errorObj[`document_${index}_url`] = `document.url should be a valid URL at index ${index}`
            }
          }
        })
      }
    ...
    if (!onUpdateOrder?.updated_at) {
      errorObj.updated_at = 'updated_at is missing'
    } else {
      const updatedDate = new Date(onUpdateOrder.updated_at)
    ...
    logger.error(`!!Error while checking on_update info in /${TRV13ApiSequence.ON_UPDATE}, ${error.stack}`)
